...
let count = 1;
let sessionEmail = localStorage.getItem('session_student_email');
function Welcome() {
    // State management for notification popup
    const [notificationPopupStatus, setNotificationPopupStatus] = useState(false);

    let loggedinAdminName = localStorage.getItem('session_student_name');
    ...
    const getAdminStudentExamDataFromDB = async () => {
        try {
            const response = await fetch('http://localhost:8080/get_admin_student_exam_count_student', {
                method: 'POST',
                headers: {
