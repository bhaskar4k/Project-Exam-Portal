...
function EditProfile(props) {
    ...
    function updateInformation() {
        ...
        const adminNewData = {
            ...
            adminOldEmail: sessionEmail
        };

        if (adminNewData.adminName === "" || adminNewData.adminName === null ||
            adminNewData.adminAge === "" || adminNewData.adminAge === null ||
            adminNewData.adminGender === "" || adminNewData.adminGender === null ||
            adminNewData.adminPhone === "" || adminNewData.adminPhone === null ||
            adminNewData.adminEmail === "" || adminNewData.adminEmail === null ||
            adminNewData.adminPassword === "" || adminNewData.adminPassword === null) {
            openNotificationPopup("2", "Please fillup all field data");
            return;
        }

        updateAdminInformation(adminNewData);

        async function updateAdminInformation(adminNewData) {
            try {
                const response = await fetch('http://localhost:8080/update_admin_info', {
                    method: 'POST',
                    headers: {
