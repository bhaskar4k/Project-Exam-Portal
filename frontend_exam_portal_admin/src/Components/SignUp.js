...
function SignUp() {
    ...
    function registerUserInDB() {
        ...
        const adminData = {
            ...
            adminAge: document.getElementById('age').value,
            adminGender: selectedGender,
            adminPhone: document.getElementById('phone').value,
            adminEmail: document.getElementById('signemail').value,
            adminPassword: document.getElementById('signpassword').value
        };

        if (adminData.adminName === "" || adminData.adminName === null ||
            adminData.adminAge === "" || adminData.adminAge === null ||
            adminData.adminGender === "" || adminData.adminGender === null ||
            adminData.adminPhone === "" || adminData.adminPhone === null ||
            adminData.adminEmail === "" || adminData.adminEmail === null ||
            adminData.adminPassword === "" || adminData.adminPassword === null) {
    ...
    function validateLoginByDB() {
        ...
        if (adminData.adminEmail === "" || adminData.adminEmail === null ||
            adminData.adminPassword === "" || adminData.adminPassword === null) {
            openNotificationPopup("2", "Please fillup all field data");
            return;
        }
        ...
        async function sendAdminData(adminData) {
            try {
                const response = await fetch('http://localhost:8080/login_of_admin', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
                ...
                let nameOfLoggedinAdmin = "", status = wholeResponse[0];
                for (let i = 2; i < wholeResponse.length; i++) {
                    nameOfLoggedinAdmin += wholeResponse[i];
                }

                if (status === "1") {
                    localStorage.setItem('session_admin_email', adminData.adminEmail);
                    localStorage.setItem('session_admin_name', nameOfLoggedinAdmin);
                    document.getElementById('loginemail').value = "";
                    document.getElementById('loginpassword').value = "";
                    window.location.reload();
                } else {
                    openNotificationPopup("0", nameOfLoggedinAdmin);
                }
            } catch (error) {
                openNotificationPopup("0", "Internal server error");
            }
